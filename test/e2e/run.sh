...
usage() {
    ...
    echo "             See: python3 ${E2E_LIB_DIR}/topology2qemuopts.py --help"
    ...
    echo "    k8s:     Kubernetes version to be installed on VM."
    echo "             Examples: k8s=1.18.10, k8s=latest. The default is none."
...
get-py-allowed() {
    ...
    py_allowed="
import re
allowed=$("$E2E_LIB_DIR/topology.py" -t "$topology_dump_file" -r "$res_allowed_file" res_allowed -o json)
...
crio_src=${crio_src:-}
crirm_src=${crirm_src:-}
memtierd_src=${memtierd_src:-$HOST_PROJECT_DIR}
runc_src=${runc_src:-}
